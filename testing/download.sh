...
read -p "Install/update Python dependencies from requirements.txt? (Y/n): " install_deps
if [[ ! "$install_deps" =~ ^[nN] ]]; then
    "$PYTHON_BIN" -m pip install -r requirements.txt
fi

read -p "Install Playwright Chromium browser binaries? (Y/n): " install_browser
...
if [ -z "$output_filename" ]; then
    "$PYTHON_BIN" mediaspace_downloader.py "$url" $debug
else
    "$PYTHON_BIN" mediaspace_downloader.py "$url" "$output_filename" $debug
fi
